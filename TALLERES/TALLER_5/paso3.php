...
function tutoriaHonor($ests)
{
    $tutoria = array();
    $honor = array();
    $resultado = array();
    foreach ($ests as $est) {
        $prom =  array_sum($est['calificaciones']) / count($est['calificaciones']);
        if ($prom < 75) {
            array_push($tutoria, $est['nombre']);
        } elseif ($prom >= 90) {
            array_push($honor, $est['nombre']);
        }
        $resultado = ['tutoria' => $tutoria, 'honor' => $honor];
    }

    return $resultado;
}

$res = tutoriaHonor($estudiantes);
echo "<br><br>";
echo "Estudiantes que necesitan Tutoria (Promedio menor a 75): " . implode(" ", $res['tutoria']) . "<br> Estudiantes del cuerpo de Honor (Promedio de 90 en adelante): " . implode(", ", $res['honor']);
